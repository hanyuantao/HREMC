...
@interface CardViewController ()<UIWebViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,CLLocationManagerDelegate,QRcodeDelegate>
{
...
//    self.emcWebView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 20)];
    ...
    if (self.isOffical) {
        ...
        urlString = [NSString stringWithFormat:@"%@?sdToken=%@&coSessionId=%@&appName=%@&phoneNumber=%@&username=%@&signature=%@&entryPoint=%@", ceshiUrl, self.sdToken, self.coSessionId, self.appName, self.phoneNumber, self.userName, self.signature, self.entryPoint];
    }
